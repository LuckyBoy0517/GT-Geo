...
class SGFormer1(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers=2, num_heads=1, 
        ...
        self.gnn2 = MPNN(aggregator_type='sum',
                    ...
                    edge_dp=edge_dp,
                    nn_dp1=nn_dp1)
        self.gnn3 = MPNN(aggregator_type='sum',
                     node_in_feats=hidden_channels,
                     node_hidden_dim= hidden_channels,
                    edge_input_dim = edge_input_dim,
                    edge_hidden_dim = edge_hidden_dim,
    ...
    def forward(self,G,node_feat, edge_attr_tensor,data):
        ...
        x5=self.gnn3(G, x, edge_attr_tensor)
        x6=self.trans_conv3(x)
        #x1=self.bn(x1)
        if self.use_graph:
            #x2=self.gnn(G, node_feat, edge_attr_tensor)#加入MPNN
            if self.aggregate=='add':
                #x = x2
                x=self.graph_weight*x5+(1-self.graph_weight)*x6
            else:
